...
const ClientJobsSection: React.FC<ClientJobsSectionProps> = ({ jobs = [] }) => {
    ...
    const formatBudget = (job: any) => {
        ...
        if (min && max) {
            return `${min.toLocaleString()} KD - ${max.toLocaleString()} KD`;
        } else if (min) {
            return `${min.toLocaleString()}+ KD`;
        } else if (max) {
            return `Up to ${max.toLocaleString()} KD`;
        }
        return "Budget TBD";
    };
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={handleViewAll}>
                    <LinearGradient colors={[COLORS.accent, COLORS.accentSecondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.viewAllButton}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.jobsList}>
                {jobsArray.slice(0, 5).map((job) => (
                    <TouchableOpacity key={job._id} style={styles.jobCard} onPress={() => handleJobPress(job._id)} activeOpacity={0.7}>
