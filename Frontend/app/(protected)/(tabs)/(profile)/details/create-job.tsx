...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, TextInput, SafeAreaView, KeyboardAvoidingView, Platform, Alert } from "react-native";
...
export default function CreateJobScreen() {
    ...
    useEffect(() => {
        ...
        if (userRole !== "client") {
            Alert.alert("Access Denied", "Only clients can create jobs. Please log out and log back in to refresh your account permissions.", [
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.title || !formData.description || !formData.category || !formData.budget_min || !formData.budget_max || !formData.duration) {
                ...
                <ScrollView showsVerticalScrollIndicator={false} style={styles.content} contentContainerStyle={styles.contentContainer}>
                    <View style={styles.formSection}>
                        <View style={styles.labelRow}>
                            ...
                            <Text style={[styles.charCount, formData.title.length < 5 && formData.title.length > 0 && styles.charCountError]}>{formData.title.length}/100</Text>
                        </View>
                        <TextInput
                            style={styles.input}
                    ...
                    <View style={styles.formSection}>
                        <View style={styles.labelRow}>
                            ...
                            <Text style={[styles.charCount, formData.description.length < 10 && formData.description.length > 0 && styles.charCountError]}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            <View style={styles.categoryContainer}>
                                {categories.map((category) => (
                                    <TouchableOpacity
                                        key={category._id}
                                        style={[styles.categoryChip, formData.category === category._id && styles.categoryChipActive]}
                                        onPress={() => setFormData({ ...formData, category: category._id })}
                                    >
                                        <Text style={[styles.categoryText, formData.category === category._id && styles.categoryTextActive]}>{category.name}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </ScrollView>
              ...
              >
                ...
                <Text style={styles.radioText}>Hourly Rate</Text>
                    ...
                    <View style={styles.formSection}>
                        ...
                        <View style={styles.experienceContainer}>
                            {["entry", "intermediate", "expert"].map((level) => (
                                <TouchableOpacity
                                    key={level}
                                    style={[styles.experienceOption, formData.experienceLevel === level && styles.experienceOptionActive]}
                                    onPress={() => setFormData({ ...formData, experienceLevel: level as any })}
                                >
                                    <Text style={[styles.experienceText, formData.experienceLevel === level && styles.experienceTextActive]}>
                    ...
                    <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
                        <LinearGradient colors={[COLORS.accent, COLORS.accentSecondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.submitGradient}>
