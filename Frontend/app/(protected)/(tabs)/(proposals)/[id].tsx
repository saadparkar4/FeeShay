...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, Image, SafeAreaView, Alert, ActivityIndicator } from "react-native";
...
export default function ProposalDetailsScreen() {
    ...
    const handleAcceptProposal = async () => {
        Alert.alert("Accept Proposal", "Are you sure you want to accept this proposal? This will start the project.", [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        if (response.success) {
                            Alert.alert("Success", "Proposal accepted successfully!", [{ text: "OK", onPress: () => fetchProposal() }]);
                        }
                    } catch (error) {
            ...
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        if (response.success) {
                            Alert.alert("Success", "Proposal declined", [{ text: "OK", onPress: () => router.back() }]);
                        }
                    } catch (error) {
    ...
    const parseCoverLetter = (coverLetter: string) => {
        ...
        sections.forEach((section) => {
            if (section.startsWith("Service:")) {
                ...
                parsed.deliverables = deliverableText
                    .split("\n")
                    .map((d) => d.trim())
                    .filter((d) => d);
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={styles.content}>
                ...
                <View style={[styles.card, styles.freelancerCard]}>
                    <View style={styles.freelancerInfo}>
                        <Image source={{ uri: "https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-3.jpg" }} style={styles.avatar} />
                        <View style={styles.freelancerDetails}>
                            <View style={styles.nameRow}>
                                <Text style={styles.freelancerName}>{isClient ? proposal.freelancer.name : "Client"}</Text>
                                <View style={styles.ratingBadge}>
                                    <Ionicons name="star" size={14} color="#FFB800" />
                                    <Text style={styles.rating}>4.7</Text>
                                </View>
                            </View>
                            <Text style={styles.freelancerTitle}>{proposal.freelancer.bio || "Professional Freelancer"}</Text>
                ...
                <View style={[styles.card, styles.summaryCard]}>
                    ...
                    <View style={styles.metricsRow}>
                        ...
                        <View style={[styles.metricCard, { backgroundColor: COLORS.accent + "20" }]}>
                            ...
                            <Text style={[styles.metricValue, { color: COLORS.accent }]}>{proposal.proposed_price} KD</Text>
                        </View>
                    </View>
                            ...
                            >
                                <Text
                                    style={[
                                        styles.skillText,
                                        {
                                            color: index % 4 === 0 ? COLORS.accentSecondary : index % 4 === 1 ? COLORS.accentTertiary : index % 4 === 2 ? COLORS.accent : "#9333EA",
                                        },
                                    ]}
                                >
                                    {skill}
                ...
                <View style={[styles.card, styles.detailsCard]}>
                    ...
                    <View style={styles.budgetInfo}>
                        ...
                        <Text style={styles.proposalText}>My Proposal: {safeRender(proposal.proposed_price)} KD</Text>
                    </View>
                </View>
                        ...
                        {isClient ? (
                            <>
                                <View style={styles.topButtonsRow}>
                                    <TouchableOpacity style={styles.acceptButtonWrapper} onPress={handleAcceptProposal} disabled={updating}>
                                        <LinearGradient colors={[COLORS.accent, COLORS.accentSecondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.acceptButton}>
                                            {updating ? (
                                ...
                                <TouchableOpacity style={styles.declineButton} onPress={handleRejectProposal} disabled={updating}>
                ...
                {proposal.status !== "active" && (
                    <View style={[styles.card, styles.statusMessageCard]}>
                        ...
                        <Text style={styles.statusMessageTitle}>
                            {proposal.status === "completed" ? "Proposal Accepted!" : proposal.status === "rejected" ? "Proposal Declined" : "Proposal Cancelled"}
                        </Text>
